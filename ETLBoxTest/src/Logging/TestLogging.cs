...
namespace ALE.ETLBoxTest {
    ...
    public class TestLogging {
        ...
        [TestMethod]
        public void TestHashValuesAreEqualFor() {            
            SqlTask.ExecuteNonQuery("Test Task - same name", "Select 1 as test");
            SqlTask.ExecuteNonQuery("Test Task - same name", "Select 2 as test");
            Assert.AreEqual(4, new SqlTask("Check if hash are equal", $@"select count(*) from etl.Log group by TaskHash") { DisableLogging = true }.ExecuteScalar<int>());
